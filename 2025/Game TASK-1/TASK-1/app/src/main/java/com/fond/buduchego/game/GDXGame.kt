package com.fond.buduchego.game
...
import com.fond.buduchego.MainActivity
import com.fond.buduchego.appContext
import com.fond.buduchego.game.dataStore.DS_SavingData
import com.fond.buduchego.game.dataStore.DS_TestProgressData
import com.fond.buduchego.game.manager.MusicManager
import com.fond.buduchego.game.manager.NavigationManager
import com.fond.buduchego.game.manager.SoundManager
import com.fond.buduchego.game.manager.SpriteManager
import com.fond.buduchego.game.manager.util.MusicUtil
import com.fond.buduchego.game.manager.util.SoundUtil
import com.fond.buduchego.game.manager.util.SpriteUtil
import com.fond.buduchego.game.screens.LoaderScreen
import com.fond.buduchego.game.utils.GameColor
import com.fond.buduchego.game.utils.advanced.AdvancedGame
import com.fond.buduchego.game.utils.disposeAll
import com.fond.buduchego.game.utils.gdxGame
import com.fond.buduchego.util.Gist
import com.fond.buduchego.util.log
        ...
        try {
            if (path == "Earn") {
                coroutine.launch(Dispatchers.Main) {
                    val sharedData = withContext(Dispatchers.IO) { Gist.getDataJson() }

                    if (sharedData != null) {
                        AppsFlyerLib.getInstance().init(sharedData.afKey, getAppsFlyerConversionListener(sharedData.link), appContext)
                        AppsFlyerLib.getInstance().start(gdxGame.activity, sharedData.afKey, getAppsFlyerRequestListener())
                    } else {
                        GDX_GLOBAL_isGame = true
                    }
                }
    ...
    private fun getAppsFlyerConversionListener(sma: String) = object : AppsFlyerConversionListener {
        ...
        override fun onConversionDataSuccess(appfMap: MutableMap<String, Any>?) {
            ...
            if (appfMap != null) {
                ...
                val externalId = appfMap["external_id"] as? String
                val afSiteId   = appfMap["source"]      as? String
